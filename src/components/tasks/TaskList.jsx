...
import { FaCheck, FaEdit, FaCalendarAlt, FaFlag } from 'react-icons/fa';

const TaskList = ({ tasks = [], allTasks = [], onTasksChange }) => {
  const [editingTask, setEditingTask] = useState(null);
  const [editForm, setEditForm] = useState({ title: '', description: '' });

  // Helper to persist changes and update parent
  const persist = (nextAll) => {
    localStorage.setItem("tasks", JSON.stringify(nextAll));
    window.dispatchEvent(new StorageEvent("storage", { key: "tasks", newValue: JSON.stringify(nextAll) }));
    onTasksChange?.(nextAll);
  };
  ...
  const handleStatusChange = (taskId) => {
    const nextAll = allTasks.map(t =>
      t._id === taskId
        ? { ...t, status: t.status === "complete" ? "incomplete" : "complete", updatedAt: new Date().toISOString() }
        : t
    );
    persist(nextAll);
  };
...
const saveTask = (taskId) => {
  ...
  const nextAll = allTasks.map(t =>
    t._id === taskId
      ? { ...t, title: editForm.title, description: editForm.description, updatedAt: new Date().toISOString() }
      : t
  );
